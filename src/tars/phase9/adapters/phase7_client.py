...
===================================
Read-only HTTP client for fetching Phase 7 outcome labels and
incident memory from the Neo4j operational memory graph.

Phase 7 is optional. Its unavailability must not fail evaluation
...
logger = logging.getLogger("phase9.adapters.phase7")
...
class Phase7Client:
    ...
    Reads outcome labels and incident memory for evaluation evidence.
    Phase 7 is optional; unavailability is handled gracefully.
    """
    ...
    async def get_incident_memory(
        ...
        Returns:
            Incident memory dict or None if not found.
        """
        try:
    ...
    async def get_mission_outcomes(
        ...
        Returns:
            Mission sync data or None if not found.
        """
        try:
            async with httpx.AsyncClient(timeout=self._timeout) as client:
                resp = await client.get(
